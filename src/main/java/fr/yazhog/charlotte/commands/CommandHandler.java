...
public class CommandHandler {
    ...
    public void handleCommand(MessageReceivedEvent event) {
        String prefixSplit = event.getMessage().getContentDisplay()
                .replaceFirst(charlotte.getFileUtils().getConfig().getString("prefix"), "");
        ...
        for (ICommand iCommand : iCommands) {
            if (iCommand.getCommand().equalsIgnoreCase(cmd)) {
                try {
                    ...
                    System.out.println(
                            "Hey ! I don't have the permissions to delete messages, that's why I can't delete the message when you're typing a command.");
                }
                if (!event.getMember().hasPermission(iCommand.getPerms())) return;
                iCommand.action(event, args, event.getAuthor(), event.getMember());
                iCommand.getSubCommands().stream()
                        .filter(iSubCommand -> args.length >= 1 && iSubCommand.getName().equalsIgnoreCase(args[0]))
                        .forEach(iSubCommand ->
                                iSubCommand.action(event,
